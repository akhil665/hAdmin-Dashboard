        ...
        <Block>
          <div className="d-flex flex-wrap align-items-center justify-content-between">
            ...
            <div className="caption-text text-dark">
              940 <span className="text-base">of 5000 words used.</span>
            </div>
          </div>
          ...
          <div className="fs-14px text-dark mt-2">
            ...
            <Link to={`${process.env.PUBLIC_URL}/copywriter/pricing-plans`}>Pricing &amp; Plans</Link>.
          </div>
        </Block>
        <Block>
          <div className="nk-history">
            {history.map((hitem, hindex) => {
              const itemCount = showCount;
              ...
              return (
                <React.Fragment key={Math.floor(Math.random() * 9999999)}>
                  ...
                  {hitem.variants &&
                    hitem.variants.slice(0, showCount).map((vitem, vindex) => {
                      return (
                  ...
                  {hitem.variants && hitem.variants.length > itemCount && (
                    <a
                      ...
                      href="#"
                      onClick={() =>
                        setShowCount(hitem.variants.length !== showCount ? hitem.variants.length : itemCount)
                      }
                      className="nk-history-item nk-history-expand"
                    >
                      <div className="nk-history-symbol">
                      ...
                      <div className="nk-history-content">
                        <h6 className="nk-history-symbol-text">
                          {hitem.variants.length !== showCount
                            ? `Show ${hitem.variants.length - itemCount} more history`
                            : `Hide ${hitem.variants.length - itemCount} history`}{" "}
                        </h6>
                      </div>
                    </a>
                  )}
  ...
  return (
    <Card>
      <div className="card-body">
        ...
        <p className="lead text-base">{item.content}</p>
